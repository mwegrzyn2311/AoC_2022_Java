...
public class Ex2 {
    public static void main(String[] args) throws Exception {
        ...
        int firstDividerIdx = 1;
        int secondDividerIdx = 2;
        long start = System.currentTimeMillis();
        for (TreeNode<Integer> tree : trees) {
            if (treePairSorted(Pair.of(tree, firstDivider))) {
                ++firstDividerIdx;
                ++secondDividerIdx;
            } else if (treePairSorted(Pair.of(tree, secondDivider))) {
                ++secondDividerIdx;
            }
        }
        long end = System.currentTimeMillis();
        System.out.printf("Optimized time = %dms.%n", end - start);
